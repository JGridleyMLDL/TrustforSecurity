...
class Extractor:
  ...
  def updatePresenceAndAbsenceVote(self) -> None:
       specificity = self.getSpecificity()
        ...
        self.absenceVote = numpy.log(
            1.0 - self.recall) - numpy.log(1.0 - specificity)
...
def getProbabilityVdEqualsVGivenX(dataItemDomainSlice, relevantValue, sources, probabilityCwdvGivenXwdvSlice):
    ...
    # If there is only one value extracted for a given data item, then it MUST be the correct value for that data item (not necessarily true)
    if n == 0:
        return 1.0
    for value in dataItemDomainSlice:
        for sourceIndex in range(len(sources)):
            # Assume that if a triple was never extracted for a particular source then the probability is zero
            if value in probabilityCwdvGivenXwdvSlice[sourceIndex]:
...
def getAccuracyForSource(sourceSliceOfCwdv, probabilityVdEqualsVGivenX):
    ...
    for dataItemIndex in range(len(sourceSliceOfCwdv)):
        if sourceSliceOfCwdv[dataItemIndex]:
            numerator += sourceSliceOfCwdv[dataItemIndex][1] * \
                probabilityVdEqualsVGivenX[dataItemIndex][sourceSliceOfCwdv[dataItemIndex][0]]
    ...
    for sourceIndex in range(extractorSliceOfValueCube.shape[0]):
        for dataItemIndex in range(extractorSliceOfValueCube.shape[1]):
            if(extractorSliceOfValueCube[sourceIndex][dataItemIndex]):
                # This seems wrong, double check
                if extractorSliceOfValueCube[sourceIndex][dataItemIndex] in probabilityCwdvGivenXwdv[sourceIndex][dataItemIndex]:
                    numerator += probabilityCwdvGivenXwdv[sourceIndex][dataItemIndex][extractorSliceOfValueCube[sourceIndex][dataItemIndex]]
...
def multilayer(valuesCube, sourceDataItemDomain, dataItemDomain, maxIterations):
    ...
    # Stores all extractors in list by extractor index
    extractors = numpy.array(
        [Extractor() for _ in range(valuesCube.shape[0])], dtype="object")
    # Stores all sources in list by source index
    sources = numpy.array([Source()
                          for _ in range(valuesCube.shape[1])], dtype="object")
    probabilityCwdvGivenXwdv = numpy.array([[dict() for _ in range(valuesCube.shape[2])] for _ in range(
        valuesCube.shape[1])], dtype="object")  # 2D list by source and data item index holding a map from value to P(Cwdv|Xwdv)
    # 2D list by source and data item index holding (data item index, value, P(Cwdv|Xwdv)) tuple that is argmax P(Cwdv|Xwdv)
    argmaxProbabilityCwdvGivenXwdv = numpy.array(
        [[None] * valuesCube.shape[2] for _ in range(valuesCube.shape[1])], dtype="object")
    # 1D List by data item index holding a map from value to P(Vd = v|Xd)
    probabilityVdEqualsVGivenX = numpy.array(
        [dict() for _ in range(valuesCube.shape[2])], dtype="object")
    alphas = numpy.array([[defaultdict(lambda: 0.5) for _ in range(valuesCube.shape[2])] for _ in range(
        valuesCube.shape[1])], dtype="object")  # 2D list by source and data item index holding map from value to alpha
  ...
  for _ in range(maxIterations):
        ...
        for sourceIndex in range(len(sourceDataItemDomain)):
            for dataItemIndex in range(len(sourceDataItemDomain[sourceIndex])):
                for value in sourceDataItemDomain[sourceIndex][dataItemIndex]:
                    tmp = getProbabilityCwdvGivenXwdv(
                        valuesCube[:, sourceIndex, dataItemIndex], value, extractors, alphas[sourceIndex][dataItemIndex][value])
                    probabilityCwdvGivenXwdv[sourceIndex][dataItemIndex][value] = tmp
                    if not argmaxProbabilityCwdvGivenXwdv[sourceIndex][dataItemIndex] or argmaxProbabilityCwdvGivenXwdv[sourceIndex][dataItemIndex][1] < tmp:
                        argmaxProbabilityCwdvGivenXwdv[sourceIndex][dataItemIndex] = (
                            value, tmp)
      # Estimate V
      ...
      # Estimate Accuracy
      for sourceIndex in range(len(sources)):
           sources[sourceIndex].accuracy = getAccuracyForSource(
                argmaxProbabilityCwdvGivenXwdv[sourceIndex], probabilityVdEqualsVGivenX)

      # Estimate Precision and Recall
      denominatorRecall = sum(value for values in [values.values(
      ) for values in probabilityCwdvGivenXwdv.flatten()] for value in values)
...
def main():
  ...
  valueCube = numpy.empty((len(extractorToIndex), len(sourceToIndex), len(dataItemToIndex)), dtype = "object")
   sourceDataItemDomain = numpy.array([[set() for _ in range(len(dataItemToIndex))] for _ in range(len(sourceToIndex))], dtype= "object")
  ...
  for extractor in data:
       for source in data[extractor]:
            for triple in data[extractor][source]:
                dataItem = (triple[0], triple[1])
                value = triple[2]
                valueCube[extractorToIndex[extractor],
                          sourceToIndex[source], dataItemToIndex[dataItem]] = value
                sourceDataItemDomain[sourceToIndex[source],
                                     dataItemToIndex[dataItem]].add(value)
                dataItemDomain[dataItemToIndex[dataItem]].add(value)

  print(multilayer(valueCube, sourceDataItemDomain, dataItemDomain, 1000))
