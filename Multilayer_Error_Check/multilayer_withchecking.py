...
def multilayer(valuesCube, sourceDataItemDomain, dataItemDomain, dataItemToIndex, maxIterations):
    ...
    probabilityCwdvGivenXwdv = numpy.array(
        [[dict() for _ in range(valuesCube.shape[2])]
         for _ in range(valuesCube.shape[1])],
    ...
    alphas = numpy.array(
        [[defaultdict(lambda: 0.5) for _ in range(valuesCube.shape[2])]
         for _ in range(valuesCube.shape[1])],
    ...
    error_Checking(valuesCube, sources, extractors, probabilityVdEqualsVGivenX, dataItemDomain, dataItemToIndex)
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        exit()
    '''
    with open("multilayerinput.json", 'r') as inputFile:
    ...
    valueCube = numpy.empty((len(extractorToIndex), len(
        sourceToIndex), len(dataItemToIndex)), dtype="object")
    sourceDataItemDomain = numpy.array(
        [[set() for _ in range(len(dataItemToIndex))] for _ in range(len(sourceToIndex))], dtype="object")
    dataItemDomain = numpy.array(
        [set() for _ in range(len(dataItemToIndex))], dtype="object")
    ...
    print(multilayer(valueCube, sourceDataItemDomain, dataItemDomain, dataItemToIndex, 40))
...
def calc_SqV(pVdEqualsVGivenX, IVdequalsV, dataItemDomain, dataIndex):
    # Calculates the difference between True (I) values and our (p) values
    total = 0
    for dataItemIndex in range(len(dataItemDomain)):
        d = pVdEqualsVGivenX[dataItemIndex]
        max_v = max(d, key=d.get)
        if pVdEqualsVGivenX[dataItemIndex][max_v] != IVdequalsV[dataItemIndex][2]:
            total += 1
...
def error_Checking(valuesCube, A_calc, Extractors, pVdEqualsVGivenX, dataItemDomain, dataItemToIndex):
    ...
    # Value Comparison
    with open("triples.json", 'r') as triples:
        data = json.load(triples)

    SqV = calc_SqV(pVdEqualsVGivenX, data, dataItemDomain, dataItemToIndex)
    print("Squared Error of Values (p, r): " + str(SqV))
